...
__device__ void tourRemoval(Solution* ps, Data data, int k);
...
#define ACCEPT_SCORE 1.0
#define NEW_BEST_SCORE 2.0
#define RHO 0.005 
#define BOL_CONST 1e-2
#define TEMPERATURE_SHIFT 0
...
__device__ double dist(Data data, int i, int j)
{
	if (i > data.numCus) i = 0;
	if (j > data.numCus) j = 0;
...
__device__ Solution globalBest;//to record the global best of all blocks for node pair operator, initiated in the ALNS kernel
	...
	while (k > 0) {
		...
		while (x <= ps->n) {
			...
			if (inSolution(ps, x))
				...
				value = dist(data, x, globalBest.next[x]) + dist(data, x, globalBest.prev[x]);//the value should be computed from the global best
...
__device__ void tourRemoval(Solution* ps, Data data, int k)
{
	if (threadIdx.x == 0) {
		...
		while (k > 0) {
			int tourid = curand(&state) % m;
			int idxDepot = tourid + n + 1;
			int cur = ps->next[idxDepot];
			while (k > 0 && cur >= 1 && cur <= n) {
				remove(ps, cur, data);
				k--;
				cur = ps->next[idxDepot];
...
bool restart(Solution s[], Solution* d_s, int count, int noImprovement, double best)
{
	...
	int countEqual = 0;
	for (int i = 0;i < GRID_SIZE;i++) {
		if (fabs(s[i].totalCost - best) < SMALL_DOUBLE)
			countEqual++;
	}
...
//this 2 kernel help to maintain a global best solution
__global__ void initGlobalBest(Solution *d_s)
{
	int idx = -1;
	double minCost = COST;
	for (int i = 0;i < GRID_SIZE;i++) {
		if (d_s[i].totalCost < minCost) {
			minCost = d_s[i].totalCost;
			idx = i;
		}
	}
	globalBest = createEmptySolution(d_s[idx].n, d_s[idx].m);
	copySolution(&globalBest, &d_s[idx]);
}
__global__ void deleteGlobalBest(void)
{
	deleteSolution(globalBest);
}
...
void ALNS(Solution s[], Solution* d_s, Data d_data)
{
	...
	while (tag) {
		...
		//printf("%d times invoke ALNS, global best cost %.4f with best idx %d\n",count,lastBest,bestIdx);

		initGlobalBest<<<1,1>>>(d_s);
		ALNSkernel<<<GRID_SIZE, BLOCK_SIZE >>>(d_s, d_data, d_destoryWeight, d_repairWeight);
		deleteGlobalBest<<<1,1 >>>();
		CudaCheckError();
		...
		if (newBest < lastBest&&fabs(newBest-lastBest)>SMALL_DOUBLE) {
			noImprovement = 0;
		}
		else {
			noImprovement++;
	...
	printf("ALNS kernel has been invoked %d times\n", count);
}
...
void printBest(Solution s[], Solution* d_s, Data data)
{
	...
	int* dnext;
	CudaSafeCall(cudaMalloc(&dnext, (m + n + 1) * sizeof(int)));
	int bestIdx = getBestSolutionIdx(s);
...
__device__ bool accept(Solution* pNew, Solution* pCur, int step)
{
	...
	if (costChange < 0.0) {
		...
		double p = exp(-costChange / ((double)(ALNS_REPEAT - step + TEMPERATURE_SHIFT) * BOL_CONST));
		curandState_t state;
		curand_init(clock(), blockIdx.x, 0, &state);
...
__global__ void ALNSkernel(Solution* d_s, Data data, double** destoryWeight, double** repairWeight)
{
	...
	for (int i = 0;i < ALNS_REPEAT;i++) {
		...
		if (threadIdx.x == 0) {
			...
			//printf("operator chosen: des %d re %d\n", idx_des, idx_re); normal, all operator is chosen
		}
		__syncthreads();
		...
		//	//to see what solution has been generated
		//	printf("for block 0, newS cost %.4f, cur Cost %.4f\n", newS.totalCost, pS->totalCost);
		//}
		...
		if (threadIdx.x == 0 && newS.totalCost < bestS.totalCost) {
			//new best?
			//printf("~~~~~~NEW BEST IN ALNS KERNEL\n");
	...
	if (threadIdx.x == 0) {
		//printf("this is block %d, with best value %.4f\n",blockIdx.x,bestS.totalCost);
		//printf("operator times: %d %d %d %d %d , %d %d\n", des_call[0], des_call[1], des_call[2], des_call[3], des_call[4], re_call[0], re_call[1]);

		for (int i = 0;i < NUM_DESTORY;i++) {
			desW[i] = (1.0 - RHO) * desW[i] + RHO * des_call[i] / ALNS_REPEAT * des_score[i];
		}
		for (int i = 0;i < NUM_REPAIR;i++) {
			reW[i] = (1.0 - RHO) * reW[i] + RHO * re_call[i] / ALNS_REPEAT * re_score[i];
		}
